...
export function AnalyticsClient({ transactions }: { transactions: Transaction[] }) {
  ...
  const monthlyData = useMemo(() => {
    ...
    for (const t of transactions) {
      ...
      if (!map[key]) {
        map[key] = { key, month: MONTH_NAMES[d.getMonth()], income: 0, expenses: 0 };
      }
  ...
  // Category breakdown for selected or current month
  ...
  // Trend data (daily average per month)
  ...
  return (
    <div className="p-4 lg:p-6 space-y-6 max-w-6xl mx-auto">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Тренд накоплений</CardTitle>
          </CardHeader>
          <CardContent>
            {!trendData.length ? (
              <p className="text-sm text-muted-foreground text-center py-8">Нет данных</p>
