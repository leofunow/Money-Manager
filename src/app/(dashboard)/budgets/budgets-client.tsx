...
export function BudgetsClient() {
  ...
  const { data: categories = [], isLoading: catLoading } = useCategories(householdId);
  const { data: budgets = [], isLoading: budgetsLoading } = useBudgets(householdId);
  const { data: monthTxns = [] } = useMonthSpending(accountIds);
  ...
  const isLoading = !profile || budgetsLoading;
  ...
  const typedBudgets = budgets as unknown as Array<{
    id: string; category_id: string; amount: number; period: string;
    category: { name: string; color: string; icon: string | null } | null;
  }>;

  const totalBudget = typedBudgets.reduce((s, b) => s + Number(b.amount), 0);
  const totalSpent = typedBudgets.reduce((s, b) => s + (spendingMap[b.category_id] ?? 0), 0);
                      ...
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">{b.period === "monthly" ? "В месяц" : "В год"}</p>
                          </div>
                        </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/50">
          <div className="bg-card rounded-2xl shadow-2xl w-full max-w-md p-5">
            ...
            {error && <div className="mb-4 bg-destructive/10 text-destructive text-sm rounded-lg p-3">{error}</div>}
